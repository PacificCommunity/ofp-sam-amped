...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  results='hide', echo=FALSE, warning=FALSE, message=FALSE, fig.width = 8, fig.height = 8
)
```
...
The HCR takes an estimate of the current stock biomass (scaled between 0 and 1, where 0 means there is no stock and 1 is the amount of fish when is there is no fishing, i.e. the highest it can be) and uses it to set a catch limit in the next year.
...
```{r plothcr, fig.cap="A threshold-catch shape HCR"}
  xrange <- c(0, 1)
  yrange <- c(0, 150)
...
When you start the app you should see three plotting windows and a blue arrow.
Your plots might look slightly different due to variations in the historic catches.

INSERT FIGURE OF FRONT PAGE

The two plots on the left-hand side show the history of the catch and stock biomass.
...
We start at the very beginning of 2020 and we want to use the HCR to set the catch limit for 2020.
...
Keep pressing the **Advance** button and you should see the cycle of how the HCR sets the catch limit, the catch limit affects the stock biomass, and the stock biomass is used by the HCR to set a new catch limit.
...
You should see that eventually the fishery settles down to a steady catch limit and stock abundance.
...
Note how the behaviour of the catch and biomass are different to the initial example.
...
You should see that the biomass now bounces around and is not perfectly flat (unlike before).
