...
There are three apps in the *AMPLE* package. This is the first one and is the one you should start with if you are unsure what a HCR is.

In this tutorial we will cover what an HCR is and how they are used for fisheries management.
...
An HCR should be designed so that the management objectives of the fishery have the greatest chance of being achieved. A good HCR is robust to different sorts of uncertainty (which we will discuss later on).

In this tutorial we will use a simple HCR to set the catch limit of a fishery in every year.
The HCR takes an estimate of the current stock biomass and uses it to set a catch limit in the next year.
Note that in this app the biomass is scaled between 0 and 1, where 0 means there is no stock (a bad thing) and 1 is the amount of fish when is there is no fishing, i.e. the highest it can be.
...
```{r plothcr, fig.cap="A threshold-catch shape HCR", fig.width=8, fig.height=6}
  xrange <- c(0, 1)
  yrange <- c(0, 150)
  xlab <- "Estimated biomass"
  ylab <- "Next catch limit"
  # Plot empty axes 
...
The current stock biomass level is on x-axis along the bottom.
...
The basic idea is that when stock biomass is low the catch limit is low and if the stock biomass is high the catch limit is high.
...
This is easier than all the stakeholders and managers arguing about what the catch limit should be (which takes time and energy and, more importantly, can lead to important decisions being made).

The shape of this HCR is determined by 4 parameters: *Blim*, *Belbow*, *Cmin* and *Cmax*, shown in blue in the figure.
When the estimated stock biomass is greater than *Belbow* the catch limit is set at *Cmax*.
When stock biomass is less than *Blim* the catch limit is set at *Cmin*.
When stock biomass is betwen *Blim* and *Belbow* the catch limit is set according to the slope.
...
[https://ofp-sam.shinyapps.io/amped-intro-hcr/](https://ofp-sam.shinyapps.io/amped-intro-hcr/)
...
At the moment, there are only 10 years of catch history from 2010 to 2019 (the biomass has an extra year as it is the biomass at the *start* of the year).
...
If the stock biomass falls below this level the stock will have trouble reproducing and may be in serious trouble.
...
These are the default initial values, but make sure that they have been set OK.
We will refer to his HCR as *HCR 1*.

**Note** that as you change the HCR parameters, the shape of the HCR will change.

# Using the HCR

The purpose of the HCR is to set the catch limit each year. The HCR uses the current estimated value of stock biomass to set the next catch limit.
...
The way the HCR operates can be seen by following the blue arrow from the biomass plot, at the bottom left, to the HCR plot, at the top right.
The current estimated value of biomass is shown on the HCR plot as the blue dashed vertical line.
This shows the same value of biomass as seen in the latest biomass value in the biomass plot.

The catch limit in the following year is set by reading the corresponding catch limit from the HCR.
The catch limit is where the blue dashed vertical line hits the red HCR line (marked by the blue dot).
...
* The stock has responded to being fished in 2020. You should see that we now have an extra year in the biomass plot up to 2020. The start of the blue arrow to the right of the biomass plot has moved up or down to reflect the new estimate of biomass. The blue vertical dashed line on the HCR plot has moved left or right as well.
...
Press the **Advance** button again. The catch limit that was set for 2021 is used by the fishery. The stock responds to being fished again and we have a new estimate biomass for the beginning of 2022. This new estimate of biomass is again used by the HCR to set the new catch limit for 2022 (the blue arrow will move again to reflect this).

Keep pressing the **Advance** button until you get to the end of the projection (in 2040).
You should see the cycle of how the HCR sets the catch limit, the catch limit affects the stock biomass, and the stock biomass is used by the HCR to set a new catch limit.
...
You should see that eventually the fishery settles down to a steady catch limit and stock abundance (Figure \@ref(ref:hcr1)).
...
Keep pressing the **Advance** button until you get to the end of the projection. In the table below write down the final value of the catch and the final value of biomass that you see on the plots.
...
However, the catch limit will not be set lower unless biomass is lower than 0.3 (the value of *Belbow*).

As before, repeatedly press the **Advance** button and follow the evolution of the stock and the catches.
Note how the behaviour of the catch and biomass are different to the initial example (Figure \@ref(fig:hcr2)).
Write down the final values and any observations in the table at the end of this section.

```{r hcr2, fig.cap="The results of running a full projection of HCR 2."}
...
The catch limit will only be at this maximum catch limit if the biomass is greater than 0.8.
...
| HCR | Final biomass | Final catch | Notes |
|----:|--------:|--------:|--------:|
| HCR 1 |  |  |  |
| (Belbow=0.5, Cmax=140) |  |  |  |
| HCR 2 |  |  |  |
...
* You have now looked at the behaviour of three different HCRs. Which of these HCRs do you prefer?
* Which of them gets the biomass closest to the TRP of 0.5?
* Which one results in the highest catches?
...
Uncertainty is included in the app in two ways: biological variability in the stock dynamics (e.g. natural variability in recruitment) and through differences between the estimated level of stock biomass and the 'true' level.
...
We saw in the previous examples that, without uncertainty, eventually the stock abundance settles down to a constant value.
...
As the HCR uses the estimate of biomass to set the catch limit, it means that the catch limit set by the HCR also bounces around. Changing the catch limits then goes onto affect the stock biomass and so on.
...
This means that the HCR is not driven by the *true* value of biomass. Instead it is driven by an *estimated* value (we can never know the true value).
For example, biomass can be estimated by using a stock assessment model.
The catch limit that is set by the HCR is therefore based on estimated values of the stock biomass, not the true values.
...
You should see that the biomass plot now shows two lines. The black one is the *true* abundance and the blue one is the *estimated* abundance. It is the estimated abundance that is used by the HCR to set the catch limit.

You should see that the stock and catch bounce around. This variability is not caused by any biological variability (you have turned that off) but from the HCR using the *estimated* value of biomass to set the catch limit.
...
This means that the estimated value of SB/SBF=0 is always 10% higher than the true value, i.e. we are always overestimating the stock abundance.
...
It is very important that the selected HCR is robust to this uncertainty at that the management objectives can be achieved.
...
We have looked at three different HCRs and seen that they have different performances.
