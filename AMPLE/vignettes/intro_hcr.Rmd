...
In this tutorial we will cover what an HCR is, how they operate, and start to think about the impact of uncertainty on performance.
...
The rule is agreed by stakeholders and managers of that fishery and form part of a fisheries *harvest strategy*.
...
The HCR uses an estimate of the current stock biomass to set a catch limit in the next year
...
This is easier than all the stakeholders and managers arguing about what the catch limit should be (which takes time and energy and can delay important decisions being made).
...
* The start of the blue arrow to the right of the biomass plot has moved up or down to reflect the new current estimate of biomass. The blue vertical dashed line on the HCR plot has moved left or right as well.
...
However, this limit will only be set if the biomass is greater than 0.3 (the value of *Belbow*).
...
By uncertainty, we mean things that we are not completely sure about, for example the exact status of the stock, and also things that are predict in the future, for example future recruitment.

The projections we have run so far have not considered uncertainty (they are known as *deterministic* simulations).
This means that if we run another projection with the same HCR, the outcome will be the same.
...
As the HCR uses the estimate of biomass to set the catch limit, it means that the catch limit set by the HCR also bounces around. The catch limits set by the HCR then go onto affect the stock biomass in the same way as before.
...
You should see that the stock and catch bounce around as you project forward. This variability is not caused by any biological variability (you have turned that off) but from the HCR using the *estimated* value of biomass to set the catch limit.
...
We have seen that they can change the performance of the fishery.
