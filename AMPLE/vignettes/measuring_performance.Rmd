...
To evaluate the impact of uncertainty on the performance of an HCR, many projections are performed (sometimes hundreds).
...
On the left-side of the main panel there are time series plots of catch and biomass, similar to those seen in the *Introduction to HCRs* app (ignore the vertical dashed lines in the plots for now).
...
We will use these as performance indicators to evaluate the performance of HCRs.
...
The difference is caused by the biological variability.
...
You will see that, eventually, the average and range values in the first row of the table will not change so much with each new projection.
This means that we are starting to understand the *distribution of expected values* from this HCR, i.e. we don't know exactly what will happen in the future, but we think it will lie within that range.
...
Looking at the top row of the table and write down the average value and the range (the values in the brackets) of biomass, catch and relative CPUE in the final year.
...
Table: (\#tab:results) Results from testing the three HCRs. After running a 30 replicates, enter the average and range of values (the values in brackets) of the biomass, catch and relative CPUE in the final year.
...
This is where the ideas of prioritising PIs and evaluating trade-offs between them come in.
...
Table: (\#tab:pis) The seven performance indicators used to measure perfomance in this app.

| Performance indicator | Description |
|:-------|:-------|
| Biomass | The biomass relative to the unfished biomass.|
| Probability of being above the LRP | This reflects the of risk of the stock being overfished. |
...
To do this we are going to record the values for only three of the PIs: *Catch*, *Catch stability* and *Prob. > LRP*, in the three different time periods, in the table below (Table \@ref(tab:results2)).
