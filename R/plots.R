...
plot_majuro_all_stocks <- function(timeseries, hcr_choices, stock_params){
  # Put into one big DF
  timeseries <- bind_rows(timeseries, .id="hcr")
...
get_hcr_colours <- function(hcr_names, chosen_hcr_names){
  ...
  allcols <- colorRampPalette(brewer.pal(8,"Dark2"))(length(hcr_names))
...
plot_yieldcurve_projections <- function(stock, stock_params, app_params, draw_trajectories=FALSE){
  # x-axis = Effort
  # y-axis = Catch
  ...
  # If drawing trajectories use full range
  if (draw_trajectories){
    ...
    # Just last value
    xrange <- c(0, max(final_rel_effort * 1.1, na.rm=TRUE))
    yrange <- c(0, max(final_catch * 1.1, na.rm=TRUE))
  }
...
plot_pi_choice <- function(pis, hcr_choices, pi_choices, plot_choice){
  ...
  if (plot_choice == "PImeds"){
    ...
    p <- p + geom_boxplot(aes(ymin=q5, ymax=q95, lower=q20,middle=q50,upper=q80, fill=hcrlegend), stat="identity")
...
plot_timeseries <- function(timeseries, hcr_choices, stock_params, show_spaghetti=FALSE){
  ...
  # Add ribbons
  p <- p + geom_ribbon(aes(ymin=lower, ymax=upper, fill=hcrlegend), alpha=0.6)
